...
export const BrandDnaEditorInputSchema = z.object({
  ...
  brandValues: z.object({
    ...
    selected: z.array(z.object({
      ...
      key: z.enum(['trust', 'quality', 'speed', 'simplicity', 'luxury', 'customer_closeness', 'innovation', 'professionalism', 'transparency', 'custom']),
      label: z.string().min(1).max(60),
      iconKey: z.string().min(1).max(40),
      emotion: z.string().max(80).optional(),
      proofStyle: z.string().max(80).optional(),
      visualStyle: z.string().max(80).optional(),
      marketingStyle: z.string().max(80).optional(),
      publishingStyle: z.string().max(80).optional(),
