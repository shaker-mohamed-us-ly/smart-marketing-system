...
export interface PremiumInsightCardProps {
  icon: LucideIcon;
  title: string;
  description: string;
  badge?: string;
...
export function PremiumInsightCard({
  ...
  const badgeColor = toneColors[badgeTone];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <h3 className="text-base font-bold text-gray-900 dark:text-white leading-tight flex-1">{title}</h3>
            {badge && (
              <span
                className="inline-flex items-center px-2.5 py-1 rounded-[10px] text-xs font-semibold flex-shrink-0"
                style={{
      ...
      {/* Description */}
      <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed mb-3 line-clamp-3">{description}</p>
