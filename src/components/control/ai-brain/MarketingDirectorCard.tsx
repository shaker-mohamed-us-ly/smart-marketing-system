...
export interface MarketingDirectorCardProps extends HTMLAttributes<HTMLDivElement> {
  name?: string;
  arabicName?: string;
  status?: string;
  thinking?: string[];
...
export function MarketingDirectorCard({ 
  name: nameProp,
  ...
  className, 
  ...props 
}: MarketingDirectorCardProps) {
  const t = useTranslations('controlAIBrain.directors');
  const name = nameProp || t('marketingDirector');
  const arabicName = arabicNameProp || t('marketingDirectorArabic');
  const status = statusProp || t('analyzingPremiumPositioning');
